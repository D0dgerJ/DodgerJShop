    ...
    useEffect(() => {
        const fetchProducts = async () => {
            try {
                ...
                console.log("Fetched products:", response.data); // 👈 Логируем данные
                if (Array.isArray(response.data)) {
                    setProducts(response.data);
                } else {
                    console.error("Ошибка: API вернуло не массив!");
                }
            } catch (error) {
                                        ...
                                        <form action="" className="form-cart">
                                            <div className="item">
                                                <table id="cart-table">
                                                    ...
                                                    <tbody>
                                                        {Array.isArray(cart.items) && cart.items.length > 0 && Array.isArray(products) ? (
                                                            cart.items.map((item, index) => {
                                                                const product = Array.isArray(products)
                                                                    ? products.find((prod) => prod.product_id === item.product_id)
                                                                    : undefined;
                                                                return (
